...
from dataset.vident_test_dataset import VidentDataset
...
def renew_vident_loader(max_skip):
    vident_dataset = VidentDataset(vident_root, split='test')
    train_dataset = ConcatDataset([vident_dataset])

    print('Vident dataset size: ', len(vident_dataset))
    print('Renewed with skip: ', max_skip)
...
if para['stage'] == 0:
    ...
    increase_skip_fraction = [0.1, 0.2, 0.3, 0.4, 0.9, 1.0]
    # Vident dataset
    vident_root = path.expanduser(para['vident_root'])

    train_sampler, train_loader = renew_vident_loader(5)
    renew_loader = renew_vident_loader
